...
def choosing_problem():
	while True:
		run_choice = input("Manual(M) or simulated(S)?")
		if (run_choice == "M"):
...
def manual(): 
	...
	while(keep_playing):
		...
		print("-----------   -----------   -----------")	
		while(True):
			door_choice_input = input("Choose a door: ")
		...
		while(True):
			new_choice_input = input("Do you want to stick " + "(" + str(door_choice)+")" + " with your original choice, door " +str(door_choice) + " or switch " +"(" + str(switch_door[0]) +")" + " to door " + str(switch_door[0]) + "? ")
		...
		counter = counter + 1
		while(True):
			keep_playing_input = input("Play another round? (yes/no)")
...
if(run_choice == "M"):
	...
	while(True):
		num_loops_input = input("Please enter the number of contests - the higher the number the better the accuray:")
